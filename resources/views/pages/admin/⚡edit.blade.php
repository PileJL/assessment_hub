...
<div class="max-w-3xl mx-auto mt-4">
    ...
    <div x-data="{ confirmDelete: false }">
        ...
        <!-- Confirmation Modal -->
        <div x-show="confirmDelete" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50">
            <div @click.away="confirmDelete = false" class="bg-white rounded-xl border border-muted/30 shadow-xl max-w-sm w-full mx-4 p-6 space-y-5">
                <div>
                    <div class="text-red font-bold text-xl text-center">Delete this record?</div>
                    <p class="text-muted text-sm mt-2 text-center">This action is permanent and cannot be undone.</p>
                </div>
                <div class="flex gap-3 pt-2">
                    <button type="button" @click="confirmDelete = false" class="flex-1 font-semibold text-primary text-md rounded-lg border border-muted/30 py-2.5 hover:bg-background transition-colors">
                        Cancel
                    </button>
                    <button type="button" wire:click="delete" @click="confirmDelete = false" wire:loading.attr="disabled" class="flex-1 font-semibold text-white text-md rounded-lg bg-red py-2.5 hover:cursor-pointer flex gap-2 items-center justify-center transition-colors">
                        <span wire:loading.remove>Yes, Delete It</span>
